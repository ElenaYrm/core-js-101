...
function isInsideCircle(circle, point) {
  const xOffset = circle.center.x - point.x;
  const yOffset = circle.center.y - point.y;
  return (xOffset * xOffset + yOffset * yOffset) < circle.radius * circle.radius;
}
...
function findFirstSingleChar(str) {
  ...
  return result.includes(1) ? str[result.indexOf(1)] : null;
}
...
function isCreditCardNumber(ccn) {
  ...
  const startIndex = (str.length - 1) % 2 === 0 ? 1 : 0;
  let sum = 0;
  for (let i = 0; i < str.length; i += 1) {
    if ((i + startIndex) % 2 === 0) {
      const value = 2 * Number(str[i]);
      if (value > 9) {
        sum += value - 9;
      } else {
        sum += value;
...
function getDigitalRoot(num) {
  ...
  const sum = arr.reduce((result, item) => Number(item) + result, 0);
  if (num <= 9) return num;
...
function getCommonDirectoryPath(pathes) {
  ...
  const minItem = arr.sort((a, b) => a.length - b.length)[0];
  const part = [];
  minItem.forEach((item) => {
    if (arr.every((el) => el.includes(item))) {
      part.push(item);
    }
  });
  if (pathes.every((item) => item.startsWith('/'))) {
    if (part.length >= 0) return `${part.join('/')}/`;
  }
  return '';
}
